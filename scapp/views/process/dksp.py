...
def dksp_search(page):
    ...
    loan_apply = View_Query_Loan.query.filter('process_status=:process_status').params(process_status=PROCESS_STATUS_DQDC).paginate(page, per_page = PER_PAGE)
    return render_template("Process/dksp/dksp.html",loan_apply=loan_apply)
